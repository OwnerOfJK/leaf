...
export default function QuestionsPage() {
  ...
  const loadQuestion = useCallback(
    async (questionNum: 1 | 2 | 3) => {
      // Check if we already have this question loaded
      const existingQuestion =
        session.questions[
          `question_${questionNum}` as keyof typeof session.questions
        ];
  ...
  useEffect(() => {
    const existingAnswer =
      session.answers[
        `question_${currentQuestion}` as keyof typeof session.answers
      ];
  ...
  const currentQuestionText =
    session.questions[
      `question_${currentQuestion}` as keyof typeof session.questions
    ];
                ...
                {step < 3 && (
                  <div
                    className={`h-0.5 flex-1 mx-4 transition-all duration-500 ${
                      step < currentQuestion ? "bg-accent" : "bg-primary/20"
                    }`}
                  />
                )}
              </React.Fragment>
            ...
            <div className="space-y-4">
              ...
              {isLoadingQuestion ? (
                <div className="flex items-center gap-3 py-4">
                  ...
                  <p className="text-lg text-muted">Generating question...</p>
                </div>
              ) : (
